...
AMT_REGEX = re.compile(r'^(?:[0-9]+(?:\.[0-9]{0,2})?)?$')
D8_REGEX = re.compile(r'\*[0-9]{8}')
...
class PersonManager(models.Manager):
    ...
    def mealType_validator(self, postData):
        ...
        if len(postData['inputMealAmt']) > 11:
            errors["4error3"] = "The amount of meal card is invalid."
        if not AMT_REGEX.match(postData["inputMealAmt"]):
            errors["4error4"] = "The amount of meal card is invalid. The amount format are either 0 or 0.0 or 0.00"
        if float(postData['inputMealAmt']) > 99999999.99:
            errors["4error5"] = "The amount of meal card is too large. "
        return errors
    ...
    def parking_validator(self, postData):
        ...
        errors = {}
        if len(postData['inputPlateNo']) < 3 :
            errors["6error1"] = "The minimal of the 3 characters for the vehicle plate number."
        if len(postData['inputPlateNo']) > 7 :
            errors["6error2"] = "The maximal of the 7 characters for the vehicle plate number."
        if len(postData["inputExpiryDate"]) < 10 or not DATE_REGEX.match(postData["inputExpiryDate"]):
            errors["6error3"] = "Invalid date format in Expiration Date of parking permit."
        elif str(postData["inputExpiryDate"]) < tomorrow:
            errors["6error4"] = "Expiration Date must greater than tomorrow!"
        return errors
...
class MealCardType(models.Model):
    ...
    CardAmt = models.DecimalField(max_digits=10, decimal_places=2)
    createdAt = models.DateTimeField(auto_now_add = True)
    updatedAt = models.DateTimeField(auto_now = True)
